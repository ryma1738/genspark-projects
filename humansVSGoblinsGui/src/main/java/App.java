...
public class App {
    ...
    public static void checkSorroundings(Land map, Human player) {
        ...
        if (sorroundings.size() > 0) { // their is something near the player
            ...
            if (sorroundings.size() > 1) {
                while (true) {
                    ...
                    for (String key : directions)
                        directs += "(" + key + ") - " + sorroundings.get(key).getClass().getSimpleName() + "\n";
                    System.out.println(
            ...
            if (sorroundings.size() == 1) {
                if (sorroundings.get(directions.get(0)).getClass() == Goblin.class) {
                    if (round == 1)
                        System.out.println("You got unlucky and spawned next to a Goblin! Let the fight begin!");
                    ...
                    if (round == 1)
                        System.out.println("You got lucky and spawned next to a Treasure chest!");
